...
from app.data_handler import (create_and_add_delivery_to_db, delete_data)
...
@router.get("/get/{delivery_id}", status_code=status.HTTP_200_OK, response_model=model.DeliveryGet)
...
@router.get("/get", status_code=status.HTTP_200_OK, response_model=list[model.DeliveryGet])
...
@router.get("/create", status_code=status.HTTP_201_CREATED, response_model=model.DeliveryGet, summary="Создать новую перевозку")
